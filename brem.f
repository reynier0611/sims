...
! All together now!
	...
	if(produce_output) then
	  ...
	  write(6,*)' exp=   ',1.-dexp(dble(-1.*(bsoft)))*(1.-bhard)
	  write(6,*)' '
...
*	bhard= bhard+vac(q2)
	...
	if (produce_output) then
	  ...
	  write(6,*)' exp=     ',dexp(dble(-1.*(bsoft)))*(1.-bhard)
	  write(6,*)' 1-bhard= ',1-bhard
	  write(6,*)' exps=    ',dexp(dble(-1.*(bsoft)))
	  write(6,*)' expse=   ',dexp(dble(-1.*b))
	  write(6,*)' expsp=   ',dexp(dble(-1.*(bz+bzz)))
	  write(6,*)' '
	  write(6,*)'  Schwinger Result'
...
! ......... the derivative has dimension 1/[energy] --> convert back to MeV
	...
	if (exponentiate) then
	  bremos = -dbsoft/dexp(dble(bsoft))
	else
	  bremos = 1.-dbsoft
	endif
	if (include_hard) bremos = bremos*(1.-bhard)
	if (produce_output) then
	  write(6,*)' bremos=   ',bremos
	endif
	return
	end
