...
c problem: what about the electrons ?
...
c this is O'Brian's abbrvieated formula	  
	  Eloss_mp = 0.1536d-03 * zeff/aeff * thick * ( 19.26 +
     &          log(thick/dens) )

c this is bethe's expanded formula: 
...
c no vavilov ?!, no gaussian
	  ...
	  if(x.gt.0.0) then
c this gives an approximate landau distribution 	     
	    lambda = -2.0*log(x)
	  else
	    lambda = 100000.
	  endif
